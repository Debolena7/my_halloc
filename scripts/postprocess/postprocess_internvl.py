            ...
            for h_list in h_type:
                for k in h_list.keys():
                    ...
                    # Just warn instead of raising error
                        print(
                        f"⚠ Warning: token alignment mismatch for '{hallucinated_text}' "
                        f"in text segment '{text[start_index:end_index]}'. Skipping token_index assignment."
                        )
                        h_list[k]["token_index"] = None
                    else:
...
def main(
    input_path: str,
    tokenizer_name: str = "OpenGVLab/InternVL2-8B",
) -> None:
